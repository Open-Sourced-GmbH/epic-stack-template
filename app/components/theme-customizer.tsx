...
/**
 * Theme customizer — the navbar popover that lets a visitor re-theme the page
 * live (ADR 062). Collapsed it's a trigger pill showing the active accent;
 * expanded it's a card of controls: accent preset swatches, free Hue/Chroma/Light
 * sliders (Light clamped to the safe band), the 3-way theme segment, and the
 * button-cursor segment. It's mounted in the universal AppShell navbar on the
 * surfaces that expose the accent picker (marketing + full), subsuming the bare
 * accent swatches and theme toggle there.
 *
...
export function ThemeCustomizer({
	...
	// Anchored in the universal AppShell navbar (ADR-068): an inline trigger pill
	// that toggles a popover card hung below it (`absolute top-full right-0`),
	// rather than a page-fixed dock. The trigger owns `aria-expanded`; the card
	// renders in the header's stacking context (`z-50`) so it floats over content.
	return (
